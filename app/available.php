...
function getBookedDates()
{
    //For testing dates.
    $booked_dates = [
        ["start-date" => "2025-01-03", "end-date" => "2025-01-05"],
        ["start-date" => "2025-01-10", "end-date" => "2025-01-15"],
        ["start-date" => "2025-01-17", "end-date" => "2025-01-20"]
    ];
...
function datesToArray($booked_dates)
{
    ...
    foreach ($booked_dates as $bd) {
        $start = new DateTime($bd["start-date"]);
        $end = new DateTime($bd["end-date"]);
        while ($start <= $end) {
...
function checkAvailable($start_date, $end_date)
{
    $start_date = new DateTime($start_date);
    $end_date = new DateTime($end_date);

    $overlaps = false;
    foreach (getBookedDates() as $bd) {
        $start_bd = new DateTime($bd["start-date"]);
        $end_bd = new DateTime($bd["end-date"]);
